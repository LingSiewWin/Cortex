...
var logger, isNode2, zlib = null, brotli = null;
var init_compression = __esm(() => {
  init_logger();
  logger = getLogger("utils:compression");
...
function opsToTxData({
  ...
  logger2("txData to send as RLP %o length %d", payload, payload.length);
  ...
  try {
    ...
    if (receipt.status === "reverted") {
      try {
        ...
        logger2("Error while calling to get more details about reverted transaction. Reason: %s", reason);
    ...
    logger2("%s Detailed error stack: %o", message, error);
...
var init_extendEntity = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger6 = getLogger("actions:wallet:extend-entity");
});
...
async function mutateEntities(client, data, txParams) {
  ...
  const receipt = await sendArkivTransaction(client, txData, txParams);
  logger7("Receipt from mutateEntities %o", receipt);
...
var init_updateEntity = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger8 = getLogger("actions:wallet:update-entity");
});
...
function stampProjectAttribute(userAttributes) {
  const filtered = userAttributes.filter((a) => a.key !== PROJECT_ATTRIBUTE.key);
  return [
    { key: PROJECT_ATTRIBUTE.key, value: PROJECT_ATTRIBUTE.value },
    ...filtered
  ];
}
async function instrumentRpc(method, fn, extract3) {
  const start = performance.now();
  try {
    const result = await fn();
    const meta = extract3?.(result) ?? {};
    publish({
      type: "arkiv.rpc.call",
      ts: Date.now(),
      method,
...
async function sealPayload(key, plaintext) {
...
var require_node_gyp_build_optional_packages = __commonJS((exports, module) => {
  ...
  function load2(dir) {
    if (typeof __webpack_require__ === "function")
      return __non_webpack_require__(load2.path(dir));
    else
      return __require(load2.path(dir));
  }
...
function f32ToF16Bytes(vec) {
  ...
  for (let i = 0;i < vec.length; i++)
    u16[i] = f32ToF16(vec[i]);
  const out = new Uint8Array(u16.length * 2);
  const dv = new DataView(out.buffer);
  for (let i = 0;i < u16.length; i++)
    dv.setUint16(i * 2, u16[i], true);
  return out;
}
function encodeDocumentPayload(input) {
...
async function batchCreate(items) {
  ...
  const result = await instrumentRpc("mutateEntities", () => withRetry2(() => wallet.mutateEntities({ creates }), {
...
async function singleCreate(item) {
...
async function createMemory(item) {
...
async function createDocumentMemory(input) {
  ...
  if (input.project)
    attributes.push({ key: WORKSPACE_ATTR, value: input.project });
  if (input.sessionId)
...
var TIER_BY_ENTITY_TYPE;
var init_batch_writer = __esm(() => {
  init_arkiv_client();
  init_events();
  init_crypto();
  init_payload_key();
...
__export(exports_session_summary, {
  storeSessionSummary: () => storeSessionSummary
});
async function storeSessionSummary(input) {
...
var PER_ITEM_TIMEOUT_MS = 45000;
var MAX_ATTEMPTS = 3;
var LOCK_STALE_MS = 180000;
function pendingDir() {
...
function withTimeout2(p, ms) {
  return Promise.race([
    p,
    new Promise((_, rej) => setTimeout(() => rej(new Error("drain item timeout")), ms))
  ]);
}
...
async function main() {
  ...
  if (files.length === 0)
    return;
  const lock = join2(dir, ".drain.lock");
  if (existsSync2(lock)) {
    try {
  ...
  try {
    writeFileSync2(lock, String(process.pid), "utf-8");
  } catch {
    return;
  }
  let drained = 0;
  let failed = 0;
  try {
    const { storeSessionSummary: storeSessionSummary2 } = await Promise.resolve().then(() => (init_session_summary(), exports_session_summary));
    const { isMissingEmbeddingKey: isMissingEmbeddingKey2 } = await Promise.resolve().then(() => (init_embeddings(), exports_embeddings));
    for (const f of files) {
      const full = join2(dir, f);
      let item;
      try {
        item = JSON.parse(readFileSync2(full, "utf-8"));
      } catch {
        continue;
      }
      ...
      for (let attempt = 1;attempt <= MAX_ATTEMPTS && !ok; attempt++) {
        try {
          const res = await withTimeout2(storeSessionSummary2({
          ...
          if (isMissingEmbeddingKey2(err)) {
            log(`
${err instanceof Error ? err.message : String(err)}

(${files.length} session memory(ies) are safely queued and will sync automatically once a key is set.)`);
            failed++;
            return;
          }
