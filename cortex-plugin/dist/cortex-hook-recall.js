...
function f16ToF32(value) {
  ...
  if (exp === 0) {
    ...
    outBits = sign | 2139095040 | mant << 13;
  } else {
...
function unpackCode(bytes) {
  if (bytes.length !== PACK_SIZE) {
    throw new Error(`unpackCode: expected ${PACK_SIZE} bytes, got ${bytes.length}`);
  }
...
class Decoder {
  ...
  decodeMultiple(source, forEach) {
    ...
    try {
      let size5 = source.length;
      sequentialMode = true;
      let value = this ? this.decode(source, size5) : defaultDecoder.decode(source, size5);
      if (forEach) {
        if (forEach(value) === false) {
          return;
        }
        while (position < size5) {
...
var decoder2, src, srcEnd, position = 0, EMPTY_ARRAY, LEGACY_RECORD_INLINE_ID = 105, RECORD_DEFINITIONS_ID = 57342, RECORD_INLINE_ID = 57343, BUNDLED_STRINGS_ID = 57337, PACKED_REFERENCE_TAG_ID = 6, STOP_CODE, maxArraySize = 112810000, maxMapSize = 16810000, strings, stringPosition = 0, currentDecoder, currentStructures, srcString, srcStringStart = 0, srcStringEnd = 0, bundledStrings, referenceMap, currentExtensions, currentExtensionRanges, packedValues, dataView, restoreMapsAsObject, defaultOptions, sequentialMode = false, inlineObjectReadThreshold = 2, validName, readFixedString, readString8, readString16, readString32, isNativeAccelerationEnabled = false, fromCharCode, f32Array, u8Array, keyCache, recordDefinition = (id, structure) => {
  id = id - 57344;
  ...
  return read2();
}, SHARED_DATA_TAG_ID = 1399353956, isLittleEndianMachine, typedArrays, typedArrayTags, mult10, defaultDecoder, decode2, decodeMultiple;
...
function typedArrayEncoder(tag, size5) {
  if (!isLittleEndianMachine2 && size5 > 1)
    tag -= 4;
  return {
    tag,
    encode: function writeExtBuffer(typedArray, encode4) {
      ...
      encode4(hasNodeBuffer ? Buffer2.from(buffer2, offset, length) : new Uint8Array(buffer2, offset, length));
    }
  };
}
...
var textEncoder, extensions, extensionClasses, Buffer2, hasNodeBuffer, ByteArrayAllocate, ByteArray, MAX_STRUCTURES = 256, MAX_BUFFER_SIZE, throwOnIterable, target, targetView, position2 = 0, safeEnd, bundledStrings2 = null, MAX_BUNDLE_SIZE = 61440, hasNonLatin, RECORD_SYMBOL, Encoder, BlobConstructor, isLittleEndianMachine2, defaultEncoder, encode4, encodeAsIterable, encodeAsAsyncIterable, REUSE_BUFFER_MODE = 512, RESET_BUFFER_MODE = 1024, THROW_ON_ITERABLE = 2048;
var init_encode = __esm(() => {
  ...
  Encoder = class Encoder extends Decoder {
    constructor(options) {
      super(options);
      this.offset = 0;
      let typeBuffer;
      ...
      this.encode = function(value, encodeOptions) {
        ...
        referenceMap2 = encoder5.structuredClone ? new Map : null;
            ...
            if (constructor === Object) {
              ...
              if (encoder5.keyMap) {
                for (let [key, entryValue] of value) {
                  encode4(encoder5.encodeKey(key));
                  encode4(entryValue);
                }
              } else {
                for (let [key, entryValue] of value) {
      ...
      const writeObject = this.useRecords === false ? this.variableMapSize ? (object) => {
        ...
        if (encoder5.keyMap) {
          for (let i = 0;i < length; i++) {
            encode4(encoder5.encodeKey(keys[i]));
            encode4(vals[i]);
          }
        } else {
          for (let i = 0;i < length; i++) {
        ...
        target[objectOffset++ + start] = size5 >> 8;
        target[objectOffset + start] = size5 & 255;
      } : (object, skipValues) => {
  ...
  BlobConstructor = typeof Blob === "undefined" ? function() {} : Blob;
...
var require_elf = __commonJS((exports, module) => {
  var interpreterPath = (elf) => {
    ...
    const size5 = elf.readUInt16LE(54);
    const count = elf.readUInt16LE(56);
    for (let i = 0;i < count; i++) {
      const headerOffset = offset + i * size5;
...
var require_cbor_extract = __commonJS((exports, module) => {
  var __dirname = "/Users/lingsiewwin/Documents/Github/Arkiv/node_modules/cbor-extract";
...
var init_node_index = __esm(() => {
  ...
  init_iterators();
  init_decode();
  nativeAccelerationDisabled = process.env.CBOR_NATIVE_ACCELERATION_DISABLED !== undefined && process.env.CBOR_NATIVE_ACCELERATION_DISABLED.toLowerCase() === "true";
...
function f16BytesToF32(bytes, dim = EMBED_DIM3) {
...
function decodeDocumentPayload(bytes) {
  const obj = decode2(bytes);
  ...
  if (typeof text !== "string")
    throw new Error("decodeDocumentPayload: missing text");
  if (!(code instanceof Uint8Array))
    throw new Error("decodeDocumentPayload: missing code");
  if (!(emb instanceof Uint8Array))
    throw new Error("decodeDocumentPayload: missing emb");
  const rawSections = Array.isArray(obj.sections) ? obj.sections : [];
  const sections = rawSections.filter((s) => s.code instanceof Uint8Array).map((s) => ({
  ...
  return {
    ...
    frontmatter: obj.fm && typeof obj.fm === "object" ? obj.fm : undefined,
...
// src/lib/derivation-message.ts
function keyDerivationMessage(userAddress) {
  return [
...
var KEY_DERIVATION_DOMAIN = "CORTEX_KEY_DERIVATION_v1";

// src/constants.ts
var PROJECT_ATTRIBUTE, WORKSPACE_ATTR = "workspace", BRAGA, ENTITY_TYPE, SEALED_CONTENT_TYPE = "application/x-cortex-sealed", REINFORCEMENT, UTILITY, MARKET_ZERO_ADDRESS = "0x0000000000000000000000000000000000000000", MARKET, SESSION;
var init_constants = __esm(() => {
  ...
  if (!PROJECT_ATTRIBUTE.value) {
    throw new Error("PROJECT_ATTRIBUTE.value must be set. Without it, Cortex queries will mix with other projects' entities on the shared Arkiv DB.");
  }
  if (WORKSPACE_ATTR === PROJECT_ATTRIBUTE.key) {
    throw new Error("WORKSPACE_ATTR must differ from PROJECT_ATTRIBUTE.key \u2014 reusing the namespace key collides and silently drops workspace provenance.");
  }
  BRAGA = {
    chainId: 60138453102,
  ...
  REINFORCEMENT = {
    initialWorkingSeconds: 60 * 60,
    workingReinforcementSeconds: 24 * 60 * 60,
    episodicReinforcementSeconds: 7 * 24 * 60 * 60,
    semanticInitialSeconds: 365 * 24 * 60 * 60,
    documentInitialSeconds: 365 * 24 * 60 * 60,
    promoteToEpisodic: 2,
    promoteToSemantic: 5,
  ...
  MARKET = {
    defaultListingPriceWei: 5000000000000000n,
    seededAgentCount: 3,
    contractAddress: process.env["MARKET_CONTRACT_ADDRESS"]?.toLowerCase() ?? MARKET_ZERO_ADDRESS
  };
  SESSION = {
    defaultValidDurationSeconds: 4 * 60 * 60,
...
// src/lib/crypto.ts
async function derivePayloadKey(signatureHex) {
  const seed = toArrayBuffer(hexToBytes3(signatureHex));
...
async function openPayload(key, sealed) {
...
var DERIVATION_INFO_PAYLOAD = "cortex.payload.aes256gcm", NONCE_BYTES = 12;
...
// src/lib/eip712.ts
var VERIFYING_CONTRACT_V1 = "0x0000000000000000000000000000000000000000", CORTEX_DOMAIN_SALT, CORTEX_EIP712_DOMAIN;
var init_eip7122 = __esm(() => {
  init__esm();
  init_constants();
  CORTEX_DOMAIN_SALT = keccak256(toBytes(`${BRAGA.chainId}:${PROJECT_ATTRIBUTE.value}`));
...
function getColorForNamespace(namespace) {
  ...
  const colorIndex = Math.abs(hash3) % COLOR_PALETTE.length;
...
var ROOT_NAMESPACE = "arkiv", enabledNamespaces, disabledNamespaces, COLOR_PALETTE, isNode, ANSI_COLORS, ANSI_RESET = "\x1B[0m", ANSI_BOLD = "\x1B[1m", namespaceColors, getLogger = (scope) => {
...
var init_logger = __esm(() => {
  ...
  namespaceColors = new Map;
});

// node_modules/@arkiv-network/sdk/src/actions/public/getBlockTiming.ts
...
var logger;
var init_getBlockTiming = __esm(() => {
  init_logger();
  logger = getLogger("actions:public:get-block-timing");
});

// node_modules/@arkiv-network/sdk/src/errors.ts
var EntityMutationError, NoMoreResultsError, NoCursorOrLimitError, NoEntityFoundError;
var init_errors4 = __esm(() => {
  EntityMutationError = class EntityMutationError extends Error {
    constructor(message) {
      super(message);
      this.name = "EntityMutationError";
    }
  };
  NoMoreResultsError = class NoMoreResultsError extends Error {
    constructor() {
      super("No more results");
      this.name = "NoMoreResultsError";
    }
  };
  NoCursorOrLimitError = class NoCursorOrLimitError extends Error {
    constructor() {
      super("Cursor and limit must be defined to fetch next");
      this.name = "NoCursorOrLimitError";
    }
  };
  NoEntityFoundError = class NoEntityFoundError extends Error {
    constructor() {
...
// node_modules/@arkiv-network/sdk/src/types/entity.ts
class Entity {
  key;
  contentType;
  owner;
  ...
  payload;
  attributes;
  constructor(key, contentType = undefined, owner = undefined, creator = undefined, expiresAtBlock = undefined, createdAtBlock = undefined, lastModifiedAtBlock = undefined, transactionIndexInBlock = undefined, operationIndexInTransaction = undefined, payload = undefined, attributes) {
    this.key = key;
    this.owner = owner;
  ...
  toText() {
    if (this.payload === undefined) {
      throw new Error("Entity has no payload \u2013 it was probably queried without withPayload(true) via QueryBuilder");
    }
    try {
      return bytesToString(this.payload);
    } catch (e) {
      throw new Error(`Failed to convert entity payload to text: ${e instanceof Error ? e.message : String(e)}`, { cause: e });
    }
  }
  toJson() {
    ...
    try {
      ...
      throw new Error(`Failed to parse entity payload as JSON: ${e instanceof Error ? e.message : String(e)}`, { cause: e });
    }
  }
}
...
// node_modules/@arkiv-network/sdk/src/utils/entities.ts
async function entityFromRpcResult(rpcEntity) {
  logger2("entityFromRpcResult %o", rpcEntity);
  return new Entity(rpcEntity.key, rpcEntity.contentType, rpcEntity.owner, rpcEntity.creator, rpcEntity.expiresAt !== undefined ? BigInt(rpcEntity.expiresAt) : undefined, rpcEntity.createdAtBlock !== undefined ? BigInt(rpcEntity.createdAtBlock) : undefined, rpcEntity.lastModifiedAtBlock !== undefined ? BigInt(rpcEntity.lastModifiedAtBlock) : undefined, rpcEntity.transactionIndexInBlock !== undefined ? BigInt(rpcEntity.transactionIndexInBlock) : undefined, rpcEntity.operationIndexInTransaction !== undefined ? BigInt(rpcEntity.operationIndexInTransaction) : undefined, rpcEntity.value !== undefined ? toBytes(rpcEntity.value) : undefined, [
...
var init_entities = __esm(() => {
  ...
  init_entity();
  init_logger();
  logger2 = getLogger("utils:entities");
});

// node_modules/@arkiv-network/sdk/src/actions/public/getEntity.ts
async function getEntity(client, key) {
  ...
  if (bytes.length !== 32) {
    throw new Error(`Invalid key length: ${bytes.length} bytes. Key must be 32 bytes (64 hex characters) long.`);
  }
...
var init_getEntity = __esm(() => {
  init__esm();
  init_errors4();
  init_entities();
});

// node_modules/@arkiv-network/sdk/src/actions/public/getEntityCount.ts
...
// node_modules/@arkiv-network/sdk/src/actions/public/query.ts
async function query(client, query2, queryOptions) {
  const rpcQueryOptions = {
    includeData: {
      key: true,
...
// node_modules/@arkiv-network/sdk/src/actions/public/subscribeEntityEvents.ts
async function subscribeEntityEvents(client, {
  onError,
  onEntityCreated,
  onEntityUpdated,
  onEntityDeleted,
  ...
  const unsubscribe = client.watchEvent({
    ...
    events: arkivABI,
    onLogs: (logs) => {
      logger3("logs from subscribeEntityEvents %o", logs);
      for (const log of logs) {
        ...
        logger3("event from subscribeEntityEvents %o", event);
...
var logger3, arkivABI;
var init_subscribeEntityEvents = __esm(() => {
  init__esm();
  init_logger();
  logger3 = getLogger("actions:public:subscribe-entity-events");
  arkivABI = parseAbi([
...
// node_modules/@arkiv-network/sdk/src/query/engine.ts
function processPredicates(predicates) {
  ...
  return predicates.map((predicate) => {
    switch (predicate.type) {
      ...
      case "or":
        return `(${predicate.predicates.map((predicate2) => processPredicates([predicate2])).join(" || ")})`;
      case "and":
        return `(${predicate.predicates.map((predicate2) => processPredicates([predicate2])).join(" && ")})`;
      default:
        return "";
...
async function processQuery(client, queryParams) {
  ...
  logger4("Processing query with params %o", {
    predicates,
    cursor,
    limit,
    ownedBy,
  ...
  let query2 = processPredicates(predicates);
  if (ownedBy) {
    query2 += ` && $owner=${ownedBy}`;
  }
  if (createdBy) {
    query2 += ` && $creator=${createdBy}`;
  }
  query2 = query2.trim();
  ...
  logger4("Built query to send %s %o", query2, {
  ...
  const result = await client.request({
    method: "arkiv_query",
    params: [query2, queryOptions]
  });
  logger4("Raw result from query %o", result);
  return result;
}
var logger4;
var init_engine = __esm(() => {
  init__esm();
  init_logger();
  logger4 = getLogger("query:engine");
});

// node_modules/@arkiv-network/sdk/src/query/queryResult.ts
class QueryResult {
  entities;
  _endOfIteration;
  _cursor;
  ...
  async next() {
    ...
    logger5("Current block height for next page %s", this._validAtBlock?.toString() ?? "unknown");
  }
  hasNextPage() {
...
var logger5;
var init_queryResult = __esm(() => {
  init_errors4();
  init_logger();
  logger5 = getLogger("query:result");
});

// node_modules/@arkiv-network/sdk/src/query/queryBuilder.ts
class QueryBuilder {
  _client;
  _ownedBy;
  _createdBy;
  ...
  orderBy(attributeNameOrOrderByAttribute, attributeType, order = "asc") {
    if (!this._orderBy) {
      this._orderBy = [];
    }
    const pushOrderByAttribute = ({ name, type, order: order2 }) => {
      this._orderBy?.push({
        name,
...
// node_modules/@arkiv-network/sdk/src/clients/decorators/arkivPublic.ts
function publicArkivActions(client) {
  return {
    getEntity: (key) => getEntity(client, key),
    query: (rawQuery, queryOptions) => query(client, rawQuery, queryOptions),
    ...
    subscribeEntityEvents: ({
      ...
      onEntityExpired,
      onEntityExpiresInExtended
    }, pollingInterval, fromBlock) => subscribeEntityEvents(client, {
      onError,
      onEntityCreated,
      onEntityUpdated,
      onEntityDeleted,
...
var init_arkivPublic = __esm(() => {
  init_getBlockTiming();
  init_getEntity();
  init_query();
  init_subscribeEntityEvents();
  init_queryBuilder();
});

// node_modules/@arkiv-network/sdk/src/clients/createPublicClient.ts
function createPublicClient(parameters) {
  ...
  return client.extend(publicActions).extend(publicArkivActions);
}
...
// node_modules/@arkiv-network/sdk/src/consts.ts
var ARKIV_ADDRESS = "0x00000000000000000000000000000061726b6976", BLOCK_TIME = 2;

// node_modules/brotli-wasm/pkg.web/brotli_wasm.js
var exports_brotli_wasm = {};
__export(exports_brotli_wasm, {
...
var wasm, heap, cachedTextDecoder, cachegetUint8Memory0 = null, heap_next, WASM_VECTOR_LEN = 0, cachedTextEncoder, encodeString3, cachegetInt32Memory0 = null, stack_pointer = 32, BrotliStreamResultCode, brotli_wasm_default;
var init_brotli_wasm = __esm(() => {
  ...
  encodeString3 = typeof cachedTextEncoder.encodeInto === "function" ? function(arg, view) {
...
var exports_index_web = {};
__export(exports_index_web, {
  default: () => index_web_default
});
var index_web_default;
var init_index_web = __esm(() => {
  init_brotli_wasm();
  index_web_default = brotli_wasm_default().then(() => exports_brotli_wasm);
});

// node_modules/@arkiv-network/sdk/src/utils/compression.ts
...
async function getBrotli() {
  if (brotli)
    return brotli;
  try {
    const brotliModule = await Promise.resolve().then(() => (init_index_web(), exports_index_web));
    brotli = brotliModule.default ? await brotliModule.default : brotliModule;
  } catch (error) {
...
async function compress2(data) {
  logger6("Compressing data size %d", data.length);
  if (isNode2) {
    const zlibModule = await getZlib();
    if (zlibModule) {
      try {
        const buffer2 = Buffer.from(data);
        const compressed = zlibModule.brotliCompressSync(buffer2);
        const result = new Uint8Array(compressed);
        logger6("Brotli compressed with zlib size %d", result.length);
        return result;
      } catch (error) {
  ...
  logger6("Brotli compressed with wasm size %d", brotliCompressed.length);
  return brotliCompressed;
}
var logger6, isNode2, zlib = null, brotli = null;
var init_compression = __esm(() => {
  init_logger();
  logger6 = getLogger("utils:compression");
  isNode2 = typeof process !== "undefined" && process.versions?.node != null;
});

// node_modules/@arkiv-network/sdk/src/utils/arkivTransactions.ts
function opsToTxData({
  creates,
  updates,
  deletes,
  ...
  const payload = [
    ...
    (extensions2 ?? []).map((item) => [
      item.entityKey,
      toHex(Math.ceil(item.expiresIn / BLOCK_TIME))
    ]),
    (ownershipChanges ?? []).map((item) => [item.entityKey, item.newOwner])
  ];
  logger7("txData to send as RLP %o length %d", payload, payload.length);
...
async function sendArkivTransaction(client, data, txParams) {
  ...
  try {
    const compressed = await compress2(toBytes(data));
    ...
    if (receipt.status === "reverted") {
      try {
        ...
        throw new EntityMutationError(`Transaction ${receipt.transactionHash} reverted. Please make sure the data for entities doesn't contain invalid characters or invalid data types.
        ...
        logger7("Error while calling to get more details about reverted transaction. Reason: %s", reason);
        throw new EntityMutationError(`Transaction ${receipt.transactionHash} reverted. Reason: ${reason}`);
      }
    }
    return receipt;
  } catch (error) {
    ...
    logger7("%s Detailed error stack: %o", message, error);
    throw new EntityMutationError(message);
  }
}
var logger7;
var init_arkivTransactions = __esm(() => {
  init__esm();
  init_errors4();
  init_compression();
  init_logger();
  logger7 = getLogger("utils:arkiv-transactions");
});

// node_modules/@arkiv-network/sdk/src/actions/wallet/changeOwnership.ts
async function changeOwnership(client, data, txParams) {
  logger8("changeOwnership %o", data);
  const txData = opsToTxData({ ownershipChanges: [data] });
  const receipt = await sendArkivTransaction(client, txData, txParams);
  logger8("Receipt from changeOwnership %o", receipt);
  return {
...
var logger8;
var init_changeOwnership = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger8 = getLogger("actions:wallet:change-ownership");
});

// node_modules/@arkiv-network/sdk/src/actions/wallet/createEntity.ts
async function createEntity(client, data, txParams) {
  logger9("createEntity %o", data);
  const txData = opsToTxData({ creates: [data] });
  const receipt = await sendArkivTransaction(client, txData, txParams);
  logger9("Receipt from createEntity %o", receipt);
  return {
    txHash: receipt.transactionHash,
...
var logger9;
var init_createEntity = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger9 = getLogger("actions:wallet:create-entity");
});

// node_modules/@arkiv-network/sdk/src/actions/wallet/deleteEntity.ts
async function deleteEntity(client, data, txParams) {
  logger10("deleteEntity %o", data);
  const txData = opsToTxData({ deletes: [data] });
  const receipt = await sendArkivTransaction(client, txData, txParams);
  logger10("Receipt from deleteEntity %o", receipt);
  return {
    txHash: receipt.transactionHash,
    entityKey: data.entityKey
  };
...
var init_deleteEntity = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger10 = getLogger("actions:wallet:delete-entity");
});

// node_modules/@arkiv-network/sdk/src/actions/wallet/extendEntity.ts
async function extendEntity(client, data, txParams) {
  logger11("extendEntity %o", data);
  const txData = opsToTxData({ extensions: [data] });
  const receipt = await sendArkivTransaction(client, txData, txParams);
  logger11("Receipt from extendEntity %o", receipt);
  return {
    txHash: receipt.transactionHash,
    entityKey: receipt.logs[0].topics[1]
  };
...
var logger11;
var init_extendEntity = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger11 = getLogger("actions:wallet:extend-entity");
});

// node_modules/@arkiv-network/sdk/src/actions/wallet/mutateEntities.ts
function parseReceipt(receipt, params) {
  ...
  for (let index2 = 0;index2 < receipt.logs.length; index2++) {
    const log = receipt.logs[index2];
    if (index2 < totalCreates) {
      createdEntities.push(log.topics[1]);
    } else if (index2 < totalCreates + totalDeletes) {
      deletedEntities.push(log.topics[1]);
    } else if (index2 < totalCreates + totalUpdates + totalDeletes) {
      updatedEntities.push(log.topics[1]);
    } else if (index2 < totalCreates + totalUpdates + totalDeletes + totalExtensions) {
      extendedEntities.push(log.topics[1]);
    } else {
      ownershipChanges.push(log.topics[1]);
    }
  }
...
async function mutateEntities(client, data, txParams) {
  ...
  const receipt = await sendArkivTransaction(client, txData, txParams);
  logger12("Receipt from mutateEntities %o", receipt);
  const { createdEntities, updatedEntities, deletedEntities, extendedEntities, ownershipChanges } = parseReceipt(receipt, data);
  return {
    txHash: receipt.transactionHash,
    createdEntities,
    updatedEntities,
...
var logger12;
var init_mutateEntities = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger12 = getLogger("actions:wallet:mutate-entities");
});

// node_modules/@arkiv-network/sdk/src/actions/wallet/updateEntity.ts
async function updateEntity(client, data, txParams) {
  logger13("updateEntity %o", data);
  const txData = opsToTxData({ updates: [data] });
  const receipt = await sendArkivTransaction(client, txData, txParams);
  logger13("Receipt from updateEntity %o", receipt);
  return {
    txHash: receipt.transactionHash,
    entityKey: receipt.logs[0].topics[1]
  };
...
var logger13;
var init_updateEntity = __esm(() => {
  init_arkivTransactions();
  init_logger();
  logger13 = getLogger("actions:wallet:update-entity");
});

// node_modules/@arkiv-network/sdk/src/clients/decorators/arkivWallet.ts
...
var init_arkivWallet = __esm(() => {
  init_changeOwnership();
  init_createEntity();
  init_deleteEntity();
  init_extendEntity();
  init_mutateEntities();
...
// node_modules/@arkiv-network/sdk/src/clients/createWalletClient.ts
function createWalletClient(parameters) {
  ...
  return client.extend(publicActions).extend(walletActions).extend(walletArkivActions);
}
...
// node_modules/@arkiv-network/sdk/src/chains/braga.ts
var braga;
var init_braga = __esm(() => {
  init__esm();
  braga = defineChain({
    id: 60138453102,
    name: "Braga",
...
// node_modules/@arkiv-network/sdk/src/chains/index.ts
var init_chains = __esm(() => {
  init_braga();
});

// node_modules/@arkiv-network/sdk/src/index.ts
var init_src = __esm(() => {
...
// node_modules/@arkiv-network/sdk/src/query/predicate.ts
function eq(key, value) {
  return { type: "eq", key, value };
}

// node_modules/@arkiv-network/sdk/src/query/index.ts
...
function getWalletClient() {
  ...
  if (!pk.startsWith("0x") || pk.length !== 66) {
    throw new Error("SESSION_KEY_PRIVATE_KEY must be a 0x-prefixed 64-hex-char string (32 bytes).");
  }
  const customRpc = process.env.CORTEX_BRAGA_RPC;
...
function cortexQuery(options) {
  const q = getPublicClient().buildQuery().where(eq(PROJECT_ATTRIBUTE.key, PROJECT_ATTRIBUTE.value));
  if (options?.createdBy === null) {} else if (options?.createdBy === undefined) {
    q.createdBy(getSessionKeyAddress());
  } else {
    q.createdBy(options.createdBy);
  }
  if (options?.ownedBy)
    q.ownedBy(options.ownedBy);
  return q;
}
var _publicClient, _walletClient;
var init_arkiv_client = __esm(() => {
  init_src();
  init_accounts2();
  init_chains();
  init_query2();
...
-- See docs/MIRROR.md \xA71 for the full investigation.
...
function migrateAddOutbox(db) {
  db.exec("CREATE TABLE IF NOT EXISTS outbox (" + "id INTEGER PRIMARY KEY AUTOINCREMENT, " + "kind TEXT NOT NULL, " + "payload_json TEXT NOT NULL, " + "status TEXT NOT NULL DEFAULT 'pending', " + "attempts INTEGER NOT NULL DEFAULT 0, " + "last_error TEXT, " + "created_at_ms INTEGER NOT NULL, " + "sent_tx_hashes TEXT, " + "citation_entity_key TEXT, " + "reconciled_at_ms INTEGER" + ");");
...
function getMemoryWeights(db, entityKeys, fallback = 1) {
  const out = new Map;
...
function migrateAddPayloadHash(db) {
  ...
  const missing = db.prepare("SELECT entity_key, payload, attributes_json FROM entities " + "WHERE payload_hash IS NULL AND payload IS NOT NULL").all();
  if (missing.length === 0)
    return;
  const update = db.prepare("UPDATE entities SET payload_hash = ? WHERE entity_key = ?");
  for (const row of missing) {
    if (!row.payload || row.payload.byteLength === 0)
      continue;
    if (row.attributes_json) {
      try {
        const attrs = JSON.parse(row.attributes_json);
        const entityType = attrs.find((a) => a.key === "entityType")?.value;
        if (entityType === "state_root")
          continue;
      } catch {}
    }
...
var DEFAULT_MIRROR_PATH = "./cortex-mirror.sqlite", _db;
var init_db = __esm(() => {
  init__esm();
  init_session_key();
  init_arkiv_client();
...
async function listMirroredEntities(filter) {
  ...
  const rows = db.prepare(`SELECT * FROM entities ${where} ORDER BY last_event_block DESC LIMIT ?`).all(...allParams);
...
function classifyError(err) {
  ...
  if (lower.includes("nonce too low") || lower.includes("replacement transaction underpriced") || lower.includes("already known")) {
  ...
  for (let attempt = 1;attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (!lastErr.retryable || attempt === maxAttempts) {
        throw new Error(`[${label}] failed after ${attempt} attempt(s) (${lastErr.category}): ${lastErr.message}`, { cause: err });
      }
      const delay = lastErr.backoffMs * Math.pow(2, attempt - 1);
      console.warn(`[${label}] attempt ${attempt}/${maxAttempts} \u2192 ${lastErr.category}, retry in ${delay}ms`);
...
// src/darwinian/recall.ts
var exports_recall = {};
__export(exports_recall, {
  recall: () => recall,
  getLastRecallRanks: () => getLastRecallRanks,
  getLastRecallK: () => getLastRecallK,
  getLastRecallIds: () => getLastRecallIds,
  _resetLastRecallIds: () => _resetLastRecallIds
});
function getLastRecallIds() {
  return new Set(_lastRecallIds);
}
function getLastRecallRanks() {
  return new Map(_lastRecallRanks);
}
function getLastRecallK() {
  return _lastRecallK;
}
function _resetLastRecallIds() {
...
function textOverlapScore(query2, ruleText) {
  const tokenize = (s) => new Set(s.toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter((t) => t.length > 2));
  const q = tokenize(query2);
  if (q.size === 0)
    return 0;
...
function pickEntityType(attrs) {
  for (const a of attrs) {
    if (a.key !== "entityType")
      continue;
    if (a.value === ENTITY_TYPE.OBSERVATION)
      return "observation";
    if (a.value === ENTITY_TYPE.EPISODE)
      return "episode";
    if (a.value === ENTITY_TYPE.RULE)
      return "rule";
    if (a.value === ENTITY_TYPE.DOCUMENT)
      return "document";
  }
  return;
}
...
async function recall(opts) {
  ...
  for (const c of candidates) {
    ...
    if (!t)
      continue;
    if (opts.entityType && t !== opts.entityType)
      continue;
      ...
      if (raw) {
        try {
          ...
          try {
            ...
            if (typeof parsed?.ruleText === "string")
              body = parsed.ruleText;
          } catch {}
    ...
    hits.push({
      ...
      entityType: t,
      score,
      expiresAtBlock: c.expiresAtBlock === undefined ? 0 : Number(c.expiresAtBlock),
      payloadPreview: preview ?? previewOf(raw),
      ...docText !== undefined ? { text: docText } : {},
      attributes: c.attributes
    });
  }
  const loadWeights = opts._deps?.loadWeights ?? (async (keys) => {
    try {
...
async function defaultFetchCandidates(entityType, project) {
  ...
  const haveForProject = project ? out.some((c) => c.attributes.some((a) => a.key === WORKSPACE_ATTR && a.value === project)) : out.length > 0;
  if (!haveForProject) {
    try {
      ...
      if (arkiv.length > 0) {
        ...
        for (const c of out) {
          if (!seen.has(c.key.toLowerCase()))
            arkiv.push(c);
          if (arkiv.length >= CANDIDATE_LIMIT)
            break;
        }
        return arkiv;
      }
    } catch (err) {
      console.error("[recall] Arkiv-direct fallback failed:", err instanceof Error ? err.message : err);
    }
  }
  return out;
}
async function fetchCandidatesFromArkiv(entityType, project) {
  const builder = cortexQuery().withAttributes(true).withMetadata(true).withPayload(true).limit(CANDIDATE_LIMIT);
  if (project)
    builder.where(eq(WORKSPACE_ATTR, project));
  if (entityType)
    builder.where(eq("entityType", entityType));
  const result = await withRetry2(() => builder.fetch(), {
...
var PREVIEW_LIMIT = 200, DEFAULT_K = 5, CANDIDATE_LIMIT = 50, RABITQ_PACK_SIZE = 198, _lastRecallIds, _lastRecallRanks, _lastRecallK = 0, PROVENANCE_BOOST;
var init_recall = __esm(() => {
  init__esm();
  init_embeddings();
  init_rabitq();
  init_document_payload();
  init_constants();
  init_events();
  init_crypto();
  init_payload_key();
...
function recentFiles(cwd, limit) {
  ...
  const walk3 = (dir, depth) => {
    ...
    for (const e of entries) {
      ...
      if (st.isDirectory())
        ...
        found.push({ name: e, mtime: st.mtimeMs });
    }
  };
  try {
    walk3(cwd, 0);
...
function pendingDir() {
  ...
  return join2(base, "pending");
}
function kickDrain(project) {
...
async function main() {
  let event = {};
  try {
    const stdin = await withTimeout2(readStdin(), 2000);
    if (stdin.trim())
...
`);
    ...
    for (const h of hits) {
      const preview = (h.text ?? h.payloadPreview ?? "").replace(/\s+/g, " ").trim().slice(0, 200);
      lines.push(`- [${h.entityType}] ${preview || "(no preview)"}`);
    }
    lines.push(`(Use the cortex_recall / cortex_act MCP tools to pull full context and cite what you use \u2014 citing reinforces a memory's lease.)`);
    if (queued > 0)
      lines.push(`(Syncing ${queued} queued summary write(s) from earlier sessions in the background.)`);
...
main().catch((err) => console.error("[cortex/recall] unexpected error (ignored):", err)).finally(() => process.exit(0));
