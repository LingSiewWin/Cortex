...
var MARKET_ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";
var MARKET = {
...
function writeConfig(config) {
  ...
  writeFileSync(tmp, JSON.stringify(full, null, 2), { mode: 384 });
  try {
    chmodSync(tmp, 384);
  } catch {}
  renameSync(tmp, path);
...
// scripts/cortex-auth.ts
function resolveConnectDir() {
  const candidates = [
    join2(import.meta.dir, "connect"),
    join2(import.meta.dir, "..", "cortex-plugin", "dist", "connect")
  ];
  return candidates.find((c) => existsSync2(join2(c, "index.html"))) ?? candidates[0];
}
var AUTH_TIMEOUT_MS = 5 * 60 * 1000;
var VALID_PROVIDERS = ["openai", "openrouter", "voyage", "cohere"];
function log(...a) {
...
async function main() {
  ...
  const handleCallback = async (req) => {
    ...
    if (body.state !== state)
      return new Response("bad state", { status: 403 });
    ...
    const provider = body.embeddingProvider && VALID_PROVIDERS.includes(body.embeddingProvider) ? body.embeddingProvider : "openai";
    done.resolve({
  ...
  if (!existsSync2(join2(connectDir, "index.html"))) {
    throw new Error(`connect page not built (looked in ${connectDir}). Run \`bun run build:plugin\` first.`);
  }
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (req.method === "POST" && url2.pathname === "/callback")
        return handleCallback(req);
      if (url2.pathname === "/favicon.ico")
        return new Response(null, { status: 204 });
      const rel = url2.pathname === "/" ? "/index.html" : url2.pathname;
      const safe = normalize(join2(connectDir, rel));
      if (!safe.startsWith(connectDir))
  ...
  log(`  encryption key derived from your wallet \xB7 config written to ~/.cortex/config.json`);
  if (!result.embeddingKey)
    log(`  (no embedding key set \u2014 add one later, or it'll prompt)`);
